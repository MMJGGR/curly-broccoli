...
export default function StepFinancial({ data, update, validate }) {
  ...
  useEffect(() => {
    const errs = {};
    if (!data.employmentStatus) errs.employmentStatus = 'Employment status is required';
    if (data.annualIncome === undefined || data.annualIncome === '' || Number(data.annualIncome) < 0) {
      errs.annualIncome = 'Income must be 0 or more';
    }
    if (data.dependents === undefined || data.dependents === '' || Number(data.dependents) < 0) {
  ...
  return (
    <div className="space-y-2">
      <div>
        ...
        />
        {errors.annualIncome && <p className="text-red-600 text-sm">{errors.annualIncome}</p>}
      </div>
      <div>
        <select
      ...
      <div>
        <input
          ...
          className="border-gray-200 focus:ring-amber-500 focus:border-amber-500 rounded-lg p-2 w-full"
        />
        {errors.dependents && <p className="text-red-600 text-sm">{errors.dependents}</p>}
      </div>
    </div>
  );
}
