describe('Advisor Onboarding - Production Ready', () => {
  beforeEach(() => {
    cy.window().then((win) => {
  ...
  it('should complete full advisor registration and onboarding flow', () => {
    const advisor = {
      email: `advisor.e2e.${Date.now()}.${Math.random().toString(36).substring(7)}@example.com`,
      password: 'testpass123'
    };

    cy.log('🎯 Starting Complete Advisor E2E Test');
    ...
    cy.get('body').then(($body) => {
      if ($body.find('input').length > 0) {
        ...
        cy.get('body').then(($serviceBody) => {
          if ($serviceBody.find('select, input').length > 0) {
            // Try to select service model options
            cy.get('select').first().then(($select) => {
              if ($select.length > 0) {
                cy.wrap($select).select('fee-only');
              }
            });
  ...
  it('should handle advisor login flow correctly', () => {
    const loginEmail = `login.advisor.${Date.now()}.${Math.random().toString(36).substring(7)}@example.com`;
  ...
  it('should verify advisor data persistence via API', () => {
    const apiTestEmail = `api.test.${Date.now()}.${Math.random().toString(36).substring(7)}@example.com`;
  ...
  it('should demonstrate advisor vs user differentiation', () => {
    ...
    cy.get('button').contains('Advisor').click({ force: true });
    cy.get('button').contains('Advisor').should('have.class').and('match', /blue-600|selected|active/);
    cy.log('✅ Advisor selection UI working');
    
    // Test user selection
    cy.get('button').contains('Individual').click({ force: true });
    cy.get('button').contains('Individual').should('have.class').and('match', /blue-600|selected|active/);
    cy.log('✅ Individual selection UI working');
    
    // Switch back to advisor
    cy.get('button').contains('Advisor').click({ force: true });
    cy.log('✅ User type toggle working correctly');
  });
});