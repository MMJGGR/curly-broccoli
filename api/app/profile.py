...
from .schemas import UserProfileCreate, UserProfileOut, DependentCreate, DependentOut, ProfileOut
...
def read_profile(current: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def update_profile(profile_in: UserProfileCreate, current: User = Depends(get_current_user), db: Session = Depends(get_db)):
    profile = current.profile
    if not profile:
...
def add_dependent(dep_in: DependentCreate, current: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def remove_dependent(dep_id: str, current: User = Depends(get_current_user), db: Session = Depends(get_db)):
